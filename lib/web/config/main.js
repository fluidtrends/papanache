...
function Config(options) {
    var entry = {}; //[...config.dev.entries(options), path.resolve(options.entryFile)]
    var chunks = {};
    // if (options.isStatic) {
    entry = { __main: path_1.default.resolve(options.entryFile) };
    options.chunks.map(function (chunkId) {
        var chunk = require(options.mainDir + "/carmel/chunks/" + chunkId + "/chunk.json");
        chunks[chunkId] = chunk;
        entry[chunkId] = path_1.default.resolve(options.stackDir, options.entry.chunk);
    });
    // }
